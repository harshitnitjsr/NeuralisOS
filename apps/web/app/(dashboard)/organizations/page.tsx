...
import { Building2, Plus, FileText, MessageSquare, Trash2, Loader2, ChevronRight, Globe, Cpu } from "lucide-react";
...
const INDUSTRIES = ["Technology", "Finance", "Healthcare", "Legal", "Retail", "Manufacturing", "Education", "Other"];
    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-zinc-400 text-sm">Create an organization, upload documents, and launch an AI chatbot for it.</p>
        </div>
        <button
          id="create-org-btn"
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-xs text-zinc-500 mb-1 block">Organization Name *</label>
                  <input
                    id="org-name-input"
                    value={form.name}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 space-y-4 text-center">
          ...
          <p className="text-zinc-500">No organizations yet. Create one to get started.</p>
          <button onClick={() => setShowCreate(true)} className="px-4 py-2 rounded-xl bg-primary/10 border border-primary/20 text-primary text-sm hover:bg-primary/20 transition-colors">
          ...
          <AnimatePresence>
            {orgs.map((org, i) => (
              <motion.div
                ...
                className="group relative flex flex-col p-5 rounded-2xl bg-white/[0.02] border border-white/8 hover:border-white/15 transition-all"
              >
                ...
                <p className="text-xs text-zinc-500 mb-4 line-clamp-2 flex-1">{org.description || "No description"}</p>

                <div className="flex items-center gap-3 mb-4 text-xs text-zinc-600">
                  <span className="flex items-center gap-1"><Globe className="w-3 h-3" /> {org.industry}</span>
                  <span className="flex items-center gap-1"><FileText className="w-3 h-3" /> {org.document_count} docs</span>
                  <span className="flex items-center gap-1"><Cpu className="w-3 h-3" /> {org.agents_enabled.length} agents</span>
                </div>

                <div className="flex gap-2">
                  <Link href={`/organizations/${org.id}`} className="flex-1">
                    <button className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-xs text-zinc-300 hover:bg-white/10 transition-colors">
                      Manage <ChevronRight className="w-3 h-3" />
                    </button>
                  </Link>
                  <Link href={`/chat?org_id=${org.id}&org_name=${encodeURIComponent(org.name)}&org_ctx=${encodeURIComponent(org.description)}`}>
                    <button className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-primary/10 border border-primary/20 text-xs text-primary hover:bg-primary/20 transition-colors">
